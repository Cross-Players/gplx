...
class _TestSetsScreenState extends ConsumerState<TestSetsScreen> {
  ...
  // Show quiz result summary for completed quiz
  Future<void> _showQuizResult(
    ...
    try {
      ...
      // Convert Map<String, int> back to Map<int, int> for selectedAnswers
      final selectedAnswers = <int, int>{};
      quizResult.selectedAnswers?.forEach((key, value) {
        selectedAnswers[int.parse(key)] = value;
      });

      // Navigate to QuizResultSummary
      Navigator.push(
        context,
        MaterialPageRoute(
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Có lỗi khi tải kết quả: $e'),
